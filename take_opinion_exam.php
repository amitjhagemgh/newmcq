    ...
    if (mysqli_num_rows($test_series_result) == 0) {
        ...
        $test_series = (int)mysqli_fetch_assoc($test_series_result)["test_series"];
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                // pre_post();
                $soft_delete_sql = "UPDATE opinion_result SET status = 0 WHERE user_id = '$user_id' AND exam_id = '$exam_id' AND test_series = '$test_series'";
                $soft_delete_result = mysqli_query($conn, $soft_delete_sql);
                $sql = "INSERT INTO opinion_result(user_id, question, answer, exam_id, test_series, status) VALUES ";
                for($i = 0; $i < count($keys); $i++) {
                    if($keys[$i] == "exam-submit") {
                        continue;
                    }
            // $user_id = $row["id"];
                    $sql .= "('$user_id', '$keys[$i]', '" . get_safe_value($conn, $values[$i]) . "', '$exam_id', '" . (int)$test_series + 1 . "', 1),";
                }
                $sql = substr($sql, 0, -1);
                if(sizeof($values) == 1) {
                    $sql .= "('$row[2]', '', '', '$exam_id', '" . (int)$test_series + 1 . "', 1);";
                }
                // echo $sql;
                // die;
                $result = mysqli_query($conn, $sql);
