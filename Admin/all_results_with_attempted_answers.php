...
GROUP BY result.user_id, result.exam_id, exam_portal.exam_name, users.email_id;";
    ...
    q.id AS question_id,
    q.questions,
    COALESCE(r.correctly_questions_attempted_id, 0) AS correctly_questions_attempted_id,
    COALESCE(r.questions_attempted_id, q.id) AS questions_attempted_id,
    r.exam_attended_time,
    COALESCE(o.answers, '0') AS answers,
    CASE 
        WHEN aa.selected_option_id IS NOT NULL THEN 'Attended'
        ELSE 'Not Attempted'
    END AS attendance_status,
    CASE
        -- WHEN aa.selected_option_id IS NULL THEN 'Not attempted yet'
        WHEN r.correctly_questions_attempted_id > 0 THEN 'Correctly answered'
        WHEN r.correctly_questions_attempted_id = 0 THEN 'Incorrectly answered'
        ELSE 'Unknown status'
    END AS dynamic_notes
FROM 
    questions q
LEFT JOIN 
    result r ON r.questions_attempted_id = q.id 
    AND r.user_id = $user_id 
    AND r.exam_id = $exam_id
LEFT JOIN 
    answer_attempts aa ON aa.question_id = q.id
LEFT JOIN 
    options o ON o.id = aa.selected_option_id
WHERE
    q.exam_id = $exam_id AND q.question_type != 'title';
    ";
                                    ...
                                    if(mysqli_num_rows($question_result) > 0) {
                                        ...
                                        <td><?php
        // echo $question_row["answers"] . " <span style='color: red;'>". $question_row["correctly_questions_attempted_id"] . "</span>";
        // echo $question_row["dynamic_notes"];
        // echo $question_row["attendance_status"];
        if($question_row["dynamic_notes"] == "Unknown status") {
            echo "Not Attempted";
        } elseif($question_row["answers"] != '0') {
            echo $question_row["answers"];
            // if($question_row["dynamic_notes"] == "Correctly answered") {
            //     echo 1;
            // } else {
            //     echo $question_row["answers"];
            // }
        } elseif ($question_row["correctly_questions_attempted_id"] == 1) {
            echo 1;
        } else {
            if($question_row["dynamic_notes"] == "Correctly answered") {
                    echo 1;
