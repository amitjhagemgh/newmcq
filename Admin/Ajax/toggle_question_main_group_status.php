...
$main_group_id = get_safe_value($conn, $_POST["mainGroupId"]);
$value = get_safe_value($conn, $_POST["value"]);

// Check if the entry already exists
$check_sql = "SELECT * FROM question_main_group_mapping WHERE question_id = '$question_id' AND main_group_id = '$main_group_id'";
$check_result = mysqli_query($conn, $check_sql);

if ($value == 1) {
    if (mysqli_num_rows($check_result) == 0) {
        // Insert only if the entry does not exist
        $sql = "INSERT INTO question_main_group_mapping (question_id, main_group_id, status) VALUES ('$question_id', '$main_group_id', '$value')";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $sql = "DELETE FROM question_main_group_mapping WHERE question_id = '$question_id' AND main_group_id = '$main_group_id'";
        if (mysqli_query($conn, $sql)) {
            echo "success";
        } else {
            echo "Error: " . mysqli_error($conn);
